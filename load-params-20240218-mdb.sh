...
CHEF_REPO="chef-repo"                         ; # Collect Chef Repo Name
CHEF_DOMAINNAME="localhost"                   ; # Collect domain name for Chef environment
...
CHEF_WORKSTATION_IP="10.0.1.6"                ; # Collect Chef Workstation IP address
CHEF_SERVER_NAME="chef-server"            ; # Collect Chef Automate Server Name (lowercase)
CHEF_SERVER_IP="10.0.1.7"                   ; # Collect Chef Automate Server IP address
CHEF_NODE1_NAME="chef-node1"                  ; # Collect Chef Node 1 Name
CHEF_NODE1_IP="10.0.1.8"                      ; # Collect Chef Node 1 IP address
...
if test `command -v firefox` && test "x$CHEF_WORKSTATION_NAME" = "x`hostname -s`"; then
...
if test `hostname -s` = "$CHEF_SERVER_NAME"; then 
  if ! test -f /etc/sysctl.conf.backup; then 
