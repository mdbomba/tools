...
if [ "$HN" = "$CHEF_WORKSTATION_NAME" ]; then 
  if [ ! -f "$HOME/.ssh/id_rsa" ] ; then echo "Creating a ssh cert pair on $HN" ; `ssh-keygen rsa` ; fi
else
  echo "Copying ssh key pair from $HN and enabling ssh cert based login on $HOSTNAME"
  cd ~/.ssh
