...
# $project: Project to be installed (chef, chef-workstation, chef-server, automate, inspec, chefdk) Default is chef (client for managed nodes)
...
if test -f "/etc/lsb-release" && grep DISTRIB_ID /etc/lsb-release >/dev/null && ! grep wrlinux /etc/lsb-release >/dev/null; then
  ...
  if test "$platform" = "linuxmint"; then 
    pv_major=`echo $platform_version | cut -d '.' -f1`
    if test "$pv_major" = "21"; then platform_version="22.04"; fi
    if test "$pv_major" = "20"; then platform_version="20.04"; fi
    platform="ubuntu" 
  fi
